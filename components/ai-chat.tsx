...
import { Send, Copy, Settings, HelpCircle, RotateCcw, ChevronLeft, X, Save, Menu, Code, MessageSquare } from "lucide-react";
      ...
      <DialogContent className="w-full h-full sm:w-[95vw] sm:max-w-6xl sm:h-[90vh] bg-black/95 backdrop-blur-xl border-0 sm:border sm:border-white/10 text-white overflow-hidden p-0 sm:rounded-lg">
        ...
        <DialogHeader className="border-b border-white/10 p-2 sm:p-4 bg-black/50 backdrop-blur-sm flex flex-row items-center justify-between">
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="sm" onClick={onClose} className="h-8 w-8 p-0 text-gray-400 hover:text-white hover:bg-white/10">
              <ChevronLeft className="h-4 w-4" />
            </Button>
            <Button variant="ghost" size="sm" onClick={toggleMobileMenu} className="h-8 w-8 p-0 text-gray-400 hover:text-white hover:bg-white/10">
          ...
          <div className="flex items-center space-x-1 sm:space-x-2">
            <Button variant="ghost" size="sm" onClick={handleClearConversation} className="h-8 w-8 p-0 text-gray-400 hover:text-white hover:bg-white/10">
        ...
        {isMobileMenuOpen && (
          <div className="sm:hidden flex flex-row space-x-2 p-2 border-b border-white/10 bg-black/50">
            <Button
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Mobile: Chat or Code View */}
          {activeView === "chat" ? (
            <div className="flex-1 flex flex-col sm:hidden">
              <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-4 bg-black/20">
                {messages.length === 0 ? (
                      ...
                      <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                        <div
                          className={`max-w-[85%] sm:max-w-[70%] rounded-2xl px-4 py-3 ${
                            message.role === "user" ? "bg-white text-black" : "bg-gray-800/60 backdrop-blur-sm text-white border border-white/10"
                          }`}
                        >
                          {message.isCode ? (
                            <div className="space-y-3">
                              <ReactMarkdown className="text-sm sm:text-base leading-relaxed">{message.content}</ReactMarkdown>
                              {generatedCode && (
              ...
              {/* Progress/Error Bar */}
              {isGenerating && (
                <div className="mb-4 bg-gray-800/60 rounded-lg p-3 border border-white/10 m-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-full bg-gray-700 rounded-full h-1.5">
                        <div className="bg-white h-1.5 rounded-full transition-all duration-300" style={{ width: `${generationProgress}%` }} />
                      </div>
                    </div>
                  </div>
                )}
              ...
              {hasError && (
                <div className="mb-4 bg-red-900/20 border border-red-500/30 rounded-lg p-3 flex items-center justify-between m-2">
                  <p className="text-red-400 text-sm">{errorMessage}</p>
                  <Button onClick={handleGenerateAI} disabled={isGenerating} className="text-xs text-white bg-red-500 hover:bg-red-600 h-6 px-2">
                    Retry
                  </Button>
                </div>
              )}
            ...
            <div className="flex-1 flex flex-col sm:hidden">
              <div className="border-b border-white/10 p-2 sm:p-4 bg-black/30">
              ...
              <div className="flex-1 overflow-y-auto p-2 sm:p-4 bg-black/20">
                <pre className="text-xs text-gray-300 whitespace-pre-wrap font-mono leading-relaxed">{generatedCode}</pre>
              </div>
              <div className="border-t border-white/10 p-2 sm:p-4 bg-black/30">
                <Button
                ...
                >
                  {isSaving ? "Saving..." : <><Save className="h-4 w-4 mr-2" /> Save Function</>}
                </Button>
              </div>
            </div>
          )}
          ...
          <div className="hidden sm:flex flex-1 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col">
              <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-black/20">
                ...
                {isGenerating && (
                  <div className="mb-4 bg-gray-800/60 rounded-lg p-3 border border-white/10 m-4">
                    <div className="flex items-center space-x-3">
                      <div className="w-4 h-4 relative">
                        <Image src="/s1-logo.png" alt="S1" width={16} height={16} className="object-contain" />
                      </div>
                ...
                {hasError && (
                  <div className="mb-4 bg-red-900/20 border border-red-500/30 rounded-lg p-3 flex items-center justify-between m-4">
                    <p className="text-red-400 text-sm">{errorMessage}</p>
                    <Button onClick={handleGenerateAI} disabled={isGenerating} className="text-xs text-white bg-red-500 hover:bg-red-600 h-6 px-2">
                      Retry
                    </Button>
                ...
                <div className="border-t border-white/10 p-4 bg-black/30 backdrop-blur-sm">
                  <div className="flex space-x-2">
                    <Textarea
                      ...
                      onChange={(e) => setAiPrompt(e.target.value)}
                      placeholder="Describe what you want to create..."
                      className="flex-1 bg-black/60 border-white/20 text-white placeholder-gray-400 resize-none min-h-[44px] max-h-32 text-base"
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && !e.shiftKey) {
                          e.preventDefault();
                          handleGenerateAI();
