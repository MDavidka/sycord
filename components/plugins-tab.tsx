  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full h-full sm:w-[95vw] sm:max-w-6xl sm:h-[90vh] bg-black/95 backdrop-blur-xl border-0 sm:border sm:border-white/10 text-white overflow-hidden p-0 sm:rounded-lg flex flex-col">
        {/* Header */}
        <DialogHeader className="border-b border-white/10 p-3 sm:p-4 bg-black/50 backdrop-blur-sm flex flex-col">
          ...
          {/* Mobile Menu */}
          {isMobileMenuOpen && (
            <div className="sm:hidden flex space-x-2 mt-2 pb-2">
              <Button
              ...
              <div ref={chatContainerRef} className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4 bg-black/20">
                {messages.length === 0 ? (
                  ...
                  messages.map((message) => (
                    <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"} p-2`}>
                      <div
